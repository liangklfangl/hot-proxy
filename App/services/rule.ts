...
export class RuleService extends EventEmitter {
  ...
  // 删除规则文件
  async deleteRuleFile(userId, name) {
  ...
  // 设置规则文件的禁用同步状态
  async setRuleFileDisableSync(userId, name, disable) {
  ...
  // 保存规则文件(可能是远程、或者本地)
  async saveRuleFile(userId, ruleFile: RuleFile) {
   ...
   * @param urlObj
   */
  getProcessRule(userId, method, urlObj): Rule | null {
  ...
  async importRemoteRuleFile(userId, url): Promise<RuleFile> {
  ...
  async fetchRemoteRuleFile(url): Promise<any> {
  ...
  async copyRuleFile(userId, name) {
